...
要修改文字，先选中 Text 事件，然后点击 Inspector 里的 **Text** 输入区并直接键入。点击中间 Stage 的文字画面也会把焦点移到这个 Text 字段。输入后事件列表和 Stage 会自动刷新；最后选择 **保存** 写入 `.rpy`。

**Speaker** 直接填姓名框要显示的名字，例如“安”，无需先在代码中定义变量。编辑器会输出 `Character("安") "台词"`。已有脚本里的 `e "台词"` 会保持使用角色变量 `e`；如果在 Inspector 里修改它的 Speaker，新内容会按显示名处理。
...
- **Auto**：文本使用 `{nw=秒数}`，对话框和文字保持显示到 Inspector 设定的秒数结束。
